...
class IdiomLearningViewModel extends StateNotifier<IdiomLearningState> {
  ...
  ) : super(const IdiomLearningState());

  Future<void> fetchExampleSentences(Idiom idiom) async {
  ...
  Future<void> updateIdiomProgress(int process, int idiomTypeID) async {
    final uid = _getCurrentUserUseCase.run().uid;
    LectureProcess lectureProcess = LectureProcess(
      lectureID: idiomTypeID,
      progress: process + 1,
      uid: uid,
    );
